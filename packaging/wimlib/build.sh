...
if [[ ! -f "$1" ]]; then
  echo "source tarball not found: $1" >&2
  exit 66
fi

source_tar_dir=$(cd "$(dirname "$1")" && pwd)
source_tar="$source_tar_dir/$(basename "$1")"

mkdir -p "$2"
output=$(cd "$2" && pwd)

[[ "$WIMLIB_SOURCE_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "unreviewed WIMLIB_SOURCE_SHA256" >&2
  exit 65
}

if command -v sha256sum >/dev/null; then
  actual=$(sha256sum "$source_tar" | awk '{print $1}')
else
  actual=$(shasum -a 256 "$source_tar" | awk '{print $1}')
fi

[[ "$actual" = "$WIMLIB_SOURCE_SHA256" ]] || {
...
case "$(uname -s)" in
  Linux)
    ...
    test "$(clang --version | head -1)" = *"version 18."*
    ;;
  Darwin)
    test "$(xcodebuild -version | tr '\n' ' ')" = "Xcode 16.4 Build version "*
    ;;
  *)
    echo "unsupported build host" >&2
    exit 65
    ;;
esac
...
(
  cd "$src"
  CC=clang ./configure --prefix=/usr "${WIMLIB_CONFIGURE_FLAGS[@]}"
  make -j2
)

case "$(uname -s)" in
  Linux)
    install -m755 "$src/.libs/libwim.so.15" "$output/libwim.so.15"
    ;;
  Darwin)
...
"$src/wimlib-imagex" capture \
  "$work/smoke-root" \
  "$output/smoke.wim" \
  --compress=none

if command -v sha256sum >/dev/null; then
  sha256sum "$output"/libwim* >"$output/SHA256SUMS"
else
  shasum -a 256 "$output"/libwim* >"$output/SHA256SUMS"
fi
