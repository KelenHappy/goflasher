...
- 兩張容易辨識的實體標籤，例如 `GOFLASHER TEST A` 與
  `GOFLASHER TEST B`；
- 盡可能使用直接連接的 USB 埠；
- 可記錄 exact artifact hash 與本地 PASS/FAIL 結果的方式。
...
- 保存在本地、將每項檢查標為 pass 或 fail 的結果表。

照片、影片、第二 reviewer、公開 evidence、上傳硬體序號與外部簽核都不是發行要求。
Maintainer 對每個 exact release candidate 負責。macOS 最小紀錄見
[最終收斂目標](../architecture/MACOS-CONVERGENCE.zh-TW.md)。
