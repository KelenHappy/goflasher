...
var catalogs = map[Locale]map[string]string{
	English: {
		...
		"stage.formatting": "Formatting", "stage.writing": "Writing", "stage.decompress_writing": "Decompressing and writing", "stage.flushing": "Synchronizing device", "stage.verifying": "Verifying", "stage.ejecting": "Safely ejecting",
	...
	TraditionalChinese: {
		...
		"stage.formatting": "正在格式化", "stage.writing": "正在寫入", "stage.decompress_writing": "正在解壓縮並寫入", "stage.flushing": "正在同步裝置", "stage.verifying": "正在驗證", "stage.ejecting": "正在安全退出",
	...
	SimplifiedChinese: {
		...
		"error.devices": "无法读取设备：%v", "log.devices": "找到 %d 个允许的 USB 设备", "device.details": "%s %s\n%.1f GB · %s\n序列号：%s · USB · 读卡器：%s · 已挂载：%s · %d 个分区",
		...
		"stage.formatting": "正在格式化", "stage.writing": "正在写入", "stage.decompress_writing": "正在解压并写入", "stage.flushing": "正在同步设备", "stage.verifying": "正在验证", "stage.ejecting": "正在安全弹出",
		...
		"log.complete": "完成：%d 字节，验证=%s，弹出=%s，耗时=%s", "metrics.complete": "平均速度：%.1f MiB/s        总耗时：%s", "action.restart": "重新开始",
	...
	Japanese: {
		...
		"error.devices": "デバイスを読み取れません：%v", "log.devices": "許可された USB デバイスが %d 台見つかりました", "device.details": "%s %s\n%.1f GB · %s\nシリアル：%s · USB · カードリーダー：%s · マウント済み：%s · %d パーティション",
		...
		"stage.formatting": "フォーマット中", "stage.writing": "書き込み中", "stage.decompress_writing": "展開して書き込み中", "stage.flushing": "デバイスを同期中", "stage.verifying": "検証中", "stage.ejecting": "安全に取り出し中",
		...
		"log.complete": "完了：%d バイト、検証=%s、取り出し=%s、経過時間=%s", "metrics.complete": "平均速度：%.1f MiB/s        合計時間：%s", "action.restart": "最初からやり直す",
