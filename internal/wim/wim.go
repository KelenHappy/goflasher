...
func bundledLibraryPathFor(executable, goos string) (string, string, error) {
	...
	if goos == "darwin" {
		...
		if filepath.Base(root) != "MacOS" || filepath.Base(contents) != "Contents" {
			return "", "", fmt.Errorf("%w: executable is not in a macOS application bundle", ErrUnsupported)
		}
		return filepath.Join(contents, "Frameworks", "libwim.15.dylib"), contents, nil
	}
	if goos == "linux" && filepath.Clean(executable) == "/usr/bin/goflasher" {
		root := filepath.Join("/usr/lib/goflasher/lib/wimlib", native.BundledVersion)
		return filepath.Join(root, "libwim.so.15"), "/usr/lib/goflasher", nil
	}
	name := "libwim.so.15"
	if goos == "windows" {
		name = "libwim-15.dll"
